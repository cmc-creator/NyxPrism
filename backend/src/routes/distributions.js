...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT b.id, b.title, b.document_name, b.status, b.created_at, b.sent_at,
...
router.get('/:id', requireAuth, async (req, res) => {
  ...
  try {
    const userId = await findOrCreateUser(req.user);
    const batch = await pool.query('SELECT id, title, document_name, status, created_at, sent_at FROM distribution_batches WHERE id = $1 AND owner_user_id = $2', [batchId, userId]);
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const batchResult = await client.query(
      `INSERT INTO distribution_batches (owner_user_id, title, document_name, document_mime, document_size, document_data, status, sent_at)
       VALUES ($1, $2, $3, 'application/pdf', $4, $5, $6, $7)
       RETURNING id, title, document_name, status, created_at, sent_at`,
      [userId, title, documentName, documentBuffer.length, documentBuffer, sendNow ? 'sent' : 'draft', sendNow ? new Date() : null],
    );
    ...
    for (const recipient of recipients) {
      ...
      const inserted = await client.query(
        ...
        [batch.id, recipient.name, recipient.email, token],
      );
  ...
  res.status(201).json({ batch, recipients: rows.map(row => ({ name: row.name, email: row.email, url: `${process.env.FRONTEND_URL || 'https://nyxprism.com'}/distribution.html?token=${row.token}` })) });
});
